...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation: torch.FloatTensor) -> distributions.Distribution:
        ...
        # else:
        dist = distributions.Normal(self.mean_net(observation), torch.exp(self.logstd)[None])
        return dist
            
        # dist =self.mean_net(torch.FloatTensor(observation).to(ptu.device))
        # return dist
    ...
    def update(self, observations, actions):
        ...
        new_acs = self(ptu.from_numpy(observations))
        acs = torch.FloatTensor(actions)
        loss = -new_acs.log_prob(acs).mean()
